...
- **文脈適応翻訳（2パス翻訳）**: 単語・セル単位の独立翻訳（パス1）だけでは文書全体の文脈が失われ精度が落ちがちな問題に対応します。「文脈を踏まえて全体を再翻訳」を実行すると、パス1の訳文一式をLLMに見渡させて文書の分野（金融・法務・医療・技術など）を推定し（パス2）、その文脈を全行の再翻訳プロンプトに埋め込みます。MoE（Mixture of Experts）モデルを使う場合、分野ヒントがプロンプト先頭にあることで該当分野に強い専門家が選ばれやすくなる効果も期待できます。
